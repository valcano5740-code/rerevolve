        ...
        try {
            ...
            if (currentEmail && currentEmail !== email.toLowerCase()) {
                ...
                vscode.window.showWarningMessage(
                    `⚠️ 주의: 현재 로그인 감지 결과 ${currentEmail}이지만, 정확하지 않을 수 있습니다. ` +
                    `${email}로 토큰이 저장됩니다. 해당 계정으로 로그인 상태인지 확인하세요.`
                );
            }
